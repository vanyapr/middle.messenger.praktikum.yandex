...
9) ~~Валидация полей~~
10) ~~Валидация форм~~
11) Удаление событий с элементов при повторном рендере
12) Переработка рендера (сейчас стыдно такое показывать)
13) Рефакторинг рендер-движка (создание дом-нод вместо рендера строки)
